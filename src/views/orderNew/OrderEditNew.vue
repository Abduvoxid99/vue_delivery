<template>
  <a-page-header :title="$t('updateOrder')" @back="() => { $router.push({ name: 'MainNewOrders', query: { status_id: 'ccb62ffb-f0e1-472e-bf32-d130bea90617' } }) }">
    <a-form-model
      class="form-row"
      :gutter="16"
      ref="orderForm"
          ...
          <a-col :span="10">
            <a-card class="card" :bordered="false" style="padding-top: 0">
              <a-row>
                ...
                <!--                <a-col v-if="orderData.phone.length >= 13" :span="24">-->
  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {
        ...
        request({
          ...
          if (res.customers.length > 0) {
            ...
            this.bools.fetchUserSpin = false
            this.orderData.client_id = undefined
            this.users = []
            this.colName = 24
            this.colButton = 0
    ...
    saveRequest (files) {
      request({
        ...
        this.$message.success(this.$t('orderUpdate'))
    ...
    getOrder (id) {
      return new Promise((resolve, reject) => {
        request({
          ...
          if (res) {
            const data = this.orderData
            data.apartment = res.apartment
            data.building = res.building
            data.floor = res.floor
            data.description = res.description
            data.name = res.client_name
            data.client_id = res.client_id
            data.phone = res.client_phone_number.slice(4)
            data.location = res.to_address
            data.payment_type = res.payment_type
            this.coordinates = [res.to_location.lat, res.to_location.long]
            setTimeout(() => {
              data.branch_id = res.steps[0].branch_id
              data.branch_address = res.steps[0].address
              data.branch_phone_number = res.steps[0].phone_number
            }, 500)
            this.products = res.steps[0].products.map(e => {
              return {
                price: e.price,
                amount: e.quantity,
                value: e.product_id,
