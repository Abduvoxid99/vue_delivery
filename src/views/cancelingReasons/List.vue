<template>
  <a-card :title="$t('reasons.of.canceling')">
...
export default {
  ...
  methods: {
      moment,
    ...mapActions(['getCancelingReasons', 'deleteCancelingReason']),
    ...
    getitems () {
      this.loading = true
      this.getCancelingReasons({ page: this.page, limit: this.limit })
        .then(res => {
          this.count = res.count ?? 0
          this.items = res?.canceling_reasons
        })
    ...
    deleteItem (item) {
      this.deleteCancelingReason(item.id)
        .then(res => {
          console.log(res)
          this.getitems()
        })
