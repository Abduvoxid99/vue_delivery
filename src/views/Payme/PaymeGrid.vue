<template>
  <div>
    <a-card title="Payme" style="width: 100%">
      <a-button type="primary" slot="extra" @click="$router.push('/setting/payme/new')">{{ $t('add') }}</a-button>
    </a-card>
        ...
        <a-table :columns="columns" :data-source="data" :pagination="false">
          ...
          <span slot="actions" slot-scope="text">
            <a-row type="flex" :gutter="[8, 8]">
              <a-col span="auto"><a-button icon="edit" @click="$router.push(`/setting/payme/${text.branch_id}`)"></a-button></a-col>
              <a-col span="auto">
                  <a-popconfirm
...
export default {
  ...
  methods: {
    deletePayme (e) {
      console.log(e)
      this.$store.dispatch('deletePayme', e.branch_id).then(res => {
        console.log(res)
        this.data = this.data.filter(el => el.branch_id !== e.branch_id)
      })
    },
      getPaymeList (params) {
          this.$store.dispatch('getPaymeList', {
            page: 1,
            limit: 100
          }).then(res => {
              console.log(res)
              this.data = res.payme_infos.map(el => ({ ...el, key: el.key }))
          })
      },
      addPayme (params) {
