<template>
  <a-modal width="800px" centered v-model="visible" @cancel="hide" :title="!editable ? $t('add_promo') : $t('edit_promo')">
...
export default {
  ...
  methods: {
    ...mapActions(['postPromo', 'getPromo', 'updatePromo', 'smsProvider']),
    hide () {
      this.visible = false
      this.clear()
    },
    ...
    saveDate () {
      this.$refs.itemCreate.validateForm().then(res => {
        console.log(res)
        this.loading = true
        this.postPromo(res.data).then(res => {
          console.log(res)
