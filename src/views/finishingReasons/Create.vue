<template>
  <a-modal width="500px" v-model="visible" @cancel="hide" title="Новая причина">
...
export default {
  ...
  methods: {
    ...mapActions(['createFinishingReason', 'updateFinishingReason']),
    hide () {
      this.visible = false
      this.resetForm()
    },
    ...
    createItem () {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          this.loading = true
          this.createFinishingReason(this.form).then(res => {
            this.hide()
