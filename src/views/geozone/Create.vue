<template>
  <a-card :title="$route.params.id ? $t('geozone.edit') : $t('geozone.create')">
...
export default {
     ...
     save () {
       ...
       if (this.name !== '' && this.polygon.geometry.getCoordinates()[0].length > 0) {
         ...
         if (this.$route.params.id) {
           this.$store.dispatch('updateGeozone', {
           ...
           this.$store.dispatch('postGeozone', _form).then(res => {
             this.$notification.success({
  ...
  created () {
    if (this.$route.params.id) {
      this.$store.dispatch('getGeozoneById', this.$route.params.id).then(res => {
        this.points = [
          res.points.map(e => {
            return [e.lat, e.lon]
          })
