    ...
    <a-form-model :colon="false" ref="ruleForm" :model="form" :rules="rules">
      <a-row type="flex">
        <a-col offset="2" span="16">
          <a-form-model-item ref="name" label="Пожалуйста, выберите филиал" prop="name">
          ...
          <a-form-model-item ref="name" label="Пожалуйста, добавьте соответствующий JOWI филиал" prop="name">
            <a-input v-model="selectedItem"></a-input>
          </a-form-model-item>
...
export default {
  ...
  methods: {
    getCrmBranches (id) {
      console.log(id)
      this.$store.dispatch('getCrmBranches', id).then(res => {
        console.log(res)
        this.list = res.branches
      })
    },
    patchBranch () {
      const form = { ...this.form, jowi_id: this.selectedItem }
      console.log(form)
